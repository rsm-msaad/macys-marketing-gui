...
def calculate_clv(
    ...
    `churn_assumption` = "optimistic" → churn at end of month (keep this
    month's revenue). `discount_assumption` = "optimistic" → paid at start
    of month (no discount applied to month 0).
    """
...
def summarize_clv(
    ...
    prompt = (
        ...
        "Write 2–3 sentences: restate the key assumptions in plain English "
        "and give the CLV. No preamble."
    )
