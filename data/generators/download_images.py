...
Pulls ~300 photos across 30 retail-relevant search terms, saves them to
`data/images/dam/`, then updates `dam_assets.filename` for the first 300
clean product/lifestyle assets so the database points at real local files.
...
Rate limit notes:
    Unsplash demo tier allows 50 requests/hour. This script makes 30 search
    requests, so it needs to run across multiple hours or use a production
    key. Search calls are throttled at ~6s apart to be a good citizen.
    Image bytes are fetched from images.unsplash.com (CDN) which is not
    API rate limited. The script skips photos that were already downloaded,
    so re-running after a rate limit error continues where it left off.
...
SEARCH_TERMS: list[str] = [
    ...
    "eyeshadow palette",
    "makeup brushes flatlay",
    "beauty flatlay minimal",
    # Skincare
    "skincare",
    ...
    "home decor",
    "gift wrapping",
    "mothers day flowers gift",
    "holiday gift set box",
    "candle home fragrance",
    # Product and retail
    "luxury shopping bag",
    "retail store display",
    "product photography white",
]
PHOTOS_PER_TERM = 10  # 30 terms x 10 = 300 images
...
def load_api_key() -> str:
    ...
    if not key:
        raise RuntimeError(
            f"UNSPLASH_ACCESS_KEY not found in {ENV_PATH}. Add `UNSPLASH_ACCESS_KEY=...` to .env at the repo root."
        )
    return key
...
def search_photos(api_key: str, query: str, per_page: int) -> list[dict]:
    for attempt in range(3):
        ...
        if response.status_code == 403 or response.status_code == 429:
            ...
            print(f"        RATE LIMITED (remaining={remaining}). Waiting 120s (attempt {attempt + 1}/3)...")
            time.sleep(120)
            continue
        response.raise_for_status()
        return response.json().get("results", [])
    print("        WARN: rate limit retries exhausted, skipping query")
    return []
...
def update_dam_assets(downloaded: list[tuple[str, str]]) -> int:
    ...
    try:
        ...
        if len(rows) < len(shuffled):
            print(f"        WARN: only {len(rows)} candidate dam_assets rows; linking the first {len(rows)}.")
