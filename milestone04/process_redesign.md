...
| 4 | Creative Production | AI (agentic) + Human review | Red + Purple | The DAM Asset Curator skill reasons about campaign mood, audience, and brand voice, then calls the `find_dam_assets` MCP tool agentically to search and curate assets. Senior Designer (Abdullah) reviews the curated shortlist. |
...
| 7 | Localization | AI (agentic) + Human review | Red + Purple | The Localization Strategist skill reasons about cultural nuances per locale, then calls `generate_locale_variants` MCP tool agentically for Spanish and Quebec French. The deterministic generator produces the full variant matrix. Production Artist (Anna) spot-checks variants side-by-side. |
