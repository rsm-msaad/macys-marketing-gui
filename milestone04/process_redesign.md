...
| 4 | Creative Production | Automation + Human review | Blue + Purple | The DAM Asset Finder automation ranks 5,000 assets by tag relevance with category boost, calls the `find_dam_assets` MCP helper for rights validation. Senior Designer (Abdullah) reviews the top-12 shortlist. |
...
| 7 | Localization | Automation + Human review | Blue + Purple | The Localization Generator automation creates up to 40 variants across 10 regions and 4 placements, calls the `generate_locale_variants` MCP helper for Spanish and Quebec French transcreation. Production Artist (Anna) spot-checks variants side-by-side. |
