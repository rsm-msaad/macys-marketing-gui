...
| 3 | SKU Selection | Automation + Human review | Blue + Purple | The SKU Recommender scores 2,000 SKUs using a weighted formula (vendor commitment 30%, margin 25%, inventory 25%, seasonality 20%). MAP-protected SKUs are auto-excluded via the `check_pricing_conflicts` MCP tool. Campaign Manager (Merna) must "Lock In" the final set before the workflow advances. |
| 4 | Creative Production | Automation + Human review | Blue + Purple | The DAM Asset Finder automation ranks 5,000 assets by tag relevance with category boost, calls the `find_dam_assets` Python helper for rights validation. Senior Designer (Abdullah) reviews the top-12 shortlist. |
...
| 7 | Localization | Automation + Human review | Blue + Purple | The Localization Generator automation creates up to 40 variants across 10 regions and 4 placements, calls the `generate_locale_variants` Python helper for Spanish and Quebec French transcreation. Production Artist (Anna) spot-checks variants side-by-side. |
...
| 10 | Reporting | AI + Human sign-off | Red + Purple | The report generator drafts an executive summary from the full audit trail across all 10 steps. Campaign Manager (Merna) finalizes and owns the final sign-off before the report reaches leadership. |
...
**Agentic MCP integration at the judgment-heavy steps.** Steps 6a (Compliance) and 6b (Brief) run in agentic mode: Claude decides when to call MCP tools like `check_pricing_conflicts` mid-reasoning rather than receiving pre-fetched results. This is a deliberate design choice — for the steps where AI judgment matters most, the model should decide what to verify, not follow a hardcoded sequence. The agentic trace is captured in the Evidence panel, showing Claude's reasoning before each tool call, the tool input/output, and how the result was incorporated. Steps outside the judgment zone (layout generation, localization, activation) continue to use the deterministic pre-fetch pattern where all context is gathered before the LLM call.
