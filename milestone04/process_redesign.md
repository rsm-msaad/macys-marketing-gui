...
| 10 | Reporting | AI + Human sign-off | Red + Purple | The report generator drafts an executive summary from the full audit trail across all 10 steps. Campaign Manager (Merna) finalizes and can send the report to the team via the `send_campaign_summary` MCP tool (Gmail SMTP integration). She owns the final sign-off before the report reaches leadership. |
