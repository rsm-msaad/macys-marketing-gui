...
# Heavy imports (faiss, sentence_transformers, torch) are deferred to
# _ensure_loaded() so the module can be imported without pulling ~400 MB
# of torch into memory at app startup. This keeps Render free tier
# (512 MB) from crashing before the first request is served.
# The SentenceTransformer model is shared with HyQRetriever via
# rag._model_cache to avoid loading the weights twice.
...
class Retriever:
    ...
    def _ensure_loaded(self) -> None:
        ...
        import faiss
        from rag._model_cache import get_embedding_model
        ...
        if not index_path.exists():
            raise FileNotFoundError(
                f"FAISS index not found at {index_path}. Build it first with: python -m rag.build_index"
            )
        if not chunks_path.exists():
            raise FileNotFoundError(
                f"chunks.json not found at {chunks_path}. Rebuild the index with: python -m rag.build_index"
            )
