...
const PERSONA_COLORS: Record<string, { bg: string; accent: string }> = {
  ...
  "ceo": { bg: "#4C1D95", accent: "#6D28D9" },
  "thales": { bg: "#312E81", accent: "#4338CA" },
};
...
export default function LandingPage() {
  ...
  const ordered = ALL_PERSONA_ORDER
  ...
  const navigate = useCallback((dir: "next" | "prev") => {
    if (isAnimatingRef.current || ordered.length === 0) return;
    isAnimatingRef.current = true;
    setActiveIndex((prev) =>
      dir === "next" ? (prev + 1) % ordered.length : (prev + ordered.length - 1) % ordered.length
    );
  ...
  // Auto-rotate every 5 seconds after hero completes
  useEffect(() => {
    if (!heroComplete || ordered.length === 0) return;
    const id = setInterval(() => navigate("next"), 5000);
    return () => clearInterval(id);
  }, [heroComplete, navigate, ordered.length]);

  const active = ordered[activeIndex];
  const colors = active ? PERSONA_COLORS[active.id] ?? { bg: "#1a1a2e", accent: "#0B7B8A" } : { bg: "#1a1a2e", accent: "#0B7B8A" };
  const isCeo = active?.id === "ceo" || active?.id === "thales";

  // Get left and right personas for carousel
  const leftIdx = ordered.length > 0 ? (activeIndex + ordered.length - 1) % ordered.length : 0;
  const rightIdx = ordered.length > 0 ? (activeIndex + 1) % ordered.length : 0;
  const leftPersona = ordered[leftIdx];
  const rightPersona = ordered[rightIdx];
  ...
  return (
    <div className="relative w-full min-h-screen overflow-hidden">
      ...
      <motion.div
        ...
        animate={{ backgroundColor: heroComplete ? colors.bg : "#0a0a12" }}
      ...
      <div className="relative z-[10] flex min-h-screen flex-col">
        ...
        <div className="flex flex-1 flex-col items-center justify-center px-6">
          ...
          {/* ===== PHASE 2: Persona Carousel ===== */}
          <AnimatePresence>
            {heroComplete && ordered.length > 0 && (
              <motion.div
                initial={{ opacity: 0, y: 40 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 1, delay: 0.2, ease: [0.25, 0.46, 0.45, 0.94] }}
                className="mt-10 sm:mt-16 w-full max-w-5xl"
              >
                {/* Carousel */}
                <div className="relative flex items-end justify-center" style={{ minHeight: "320px" }}>

                  {/* Left persona (small, blurred) */}
                  {leftPersona && (
                    <motion.button
                  ...
                  {/* Center persona (large, prominent) */}
                  {active && (
                      ...
                      >
                        ...
                        {isCeo && (
                          <motion.span
                            ...
                            className="mt-3 inline-flex items-center gap-1 rounded-full px-3 py-1 text-[10px] font-bold uppercase tracking-wider"
                            style={{ backgroundColor: `${colors.accent}30`, color: "#e9d5ff" }}
                          >
                            <Shield className="h-3 w-3" /> Co-CEO
                          </motion.span>
                        )}
                        ...
                        <motion.h2
                          ...
                          className="mt-3 font-serif text-3xl sm:text-5xl font-semibold text-white text-center"
                        >
                          {active.name}
                        </motion.h2>
                  ...
                  <div className="flex items-center gap-2">
                    {ordered.map((_, i) => (
                      <button
                        key={i}
                        type="button"
                        onClick={() => { if (!isAnimatingRef.current) setActiveIndex(i); }}
          ...
          {/* Scroll indicator (before carousel appears) */}
