...
from api.routes import auth, chat, images, impact, knowledge, personas, rag_compare, sidebar, skills, slides_admin, workflow  # noqa: E402
...
def health() -> dict:
    return {
        ...
        "ai_engine_import_error": _M3_IMPORT_ERROR,
...
app.include_router(slides_admin.router)
...
async def ai_cascade(req: ApprovalCascadeRequest) -> dict:
    """Run localization after VP approval. Activation scheduling fires later
    when the localization manager locks in translations at Step 7."""
    _check_ai_ready()
    ...
    log_ai_activity(
        ...
        summary=f"Localization complete, {variant_count} variants generated. Awaiting localization manager review.",
        campaign_id=req.campaign_id,
    )
    return {
